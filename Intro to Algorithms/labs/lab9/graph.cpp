...
string getEdgeName(const string& name1, const string& name2){
    char ch1 = name1.at(0);
    char ch2 = name2.at(0);
    string edgeName;
    if(ch1 < ch2)
...
void traverseEdge(const string& name1, const string& name2, vector<string>& edgesTraversed){
    string edgeName = getEdgeName(name1, name2);
    edgesTraversed.push_back(edgeName);
}

bool Graph::hasChildren(Vertex* currVertex){
    int temp = 0;
    for(int x = 0; x < currVertex->adj.size(); x++)
        if(!currVertex->adj[x]->visited)
            temp++;
    if(temp > 0)
        return true;
    return false;
...
bool hasBeenTraversed(const string& name1, const string& name2, const vector<string>& edgesTraversed){
    string edgeName = getEdgeName(name1, name2);
    for(int x = 0; x < edgesTraversed.size(); x++)
        if(edgesTraversed[x] == edgeName)
            return true;
    return false;
}

int Graph::getVCs(Vertex* currVertex){
    currVertex->visited = true;
    if(!hasChildren(currVertex))
        return 1;
    int numCovers = 0;
    for(int x = 0; x < currVertex->adj.size(); x++)
        if(currVertex->adj[x]->visited == false){
            numCovers += getVCs(currVertex->adj[x]);
        }
    return numCovers;
}

int Graph::getVertexCovers(){
    Vertex* startVertex = allVertices[0];
    int numCovers = 0;
    vector<string> edgesTraversed;
    startVertex->visited = true;
    for(int x = 0; x < startVertex->adj.size(); x++){
        if(startVertex->adj[x]->visited == false){
            numCovers += getVCs(startVertex->adj[x]);
        }
    }
    cout << "num covers: " << numCovers << endl;
    return numCovers;
}

string getCityName(char& ch1, char& ch2){
    if(ch2 == 'Z'){
        ch1 = static_cast<char>(ch1 + 1);
        ch2 = 'A';
    }
    else
        ch2 = static_cast<char>(ch2 + 1);
    string newName;
    newName.push_back(ch1);
    newName.push_back(ch2);
    return newName;
}

void genTSP(int numCities){
    char ch1 = 'A';
    char ch2 = 'A';
    string cityName = "AA";
    srand(time(NULL));
    ...
    ofstream output;
    output.open("TSP50.txt");
    for(int x = 0; x < numCities; x++){
        numEdges = rand() % 3 + 1;
        cout << "["<< cityName << "] connected to ";
        if(numEdges < cities.size()){
            map<int, bool> connectedCities;
            while(numEdges > 0){
                if(cities.size() < 10){
                    int cityPos = rand() % cities.size();
                    map<int, bool>::iterator itr = connectedCities.find(cityPos);
                    if(itr == connectedCities.end()){
                        connectedCities.insert(pair<int,bool>(cityPos, true));
                        cout << "[" << cities[cityPos] << "] ";
                        cityWeight = rand() % 30 + 1;
                        output << cityName << " " << cities[cityPos] << " " << cityWeight << "\n";
                        output << cities[cityPos]  << " " << cityName << " " << cityWeight << "\n";
                        numEdges--;
                    }
                }
                else{
                    int cityPos = rand() % 10 + (cities.size() -10);
                    map<int, bool>::iterator itr = connectedCities.find(cityPos);
                    if(itr == connectedCities.end()){
                        connectedCities.insert(pair<int,bool>(cityPos, true));
                        cout << "[" << cities[cityPos] << "] ";
