...
template <class T> void UndoArray<T>::set(int i, T val) const{
    ...
    if(i < m_alloc){
        ...
        std::cerr << "ERROR: index value is out of bounds" << std::endl;
        return;
    }
}
...
template <class T> void UndoArray<T>::pop_back(){
    if(m_size > 0){
        ...
        */
        ...
        std::cerr << "ERROR: array is of length 0, nothing else to remove" << std::endl;
        return;
    }
}
...
template <class T> void UndoArray<T>::taPrint(){
    std::cout << "size_    " << m_size <<std::endl;
    int max_cnt_pos = 0;
    std::cout << "counts_  ";
    for(int x = 0; x < (int)m_size; ++x){
        if(counts_[x] > max_cnt_pos) max_cnt_pos = counts_[x];
        std::cout <<  counts_[x] << " ";
    }
    std::cout<< std::endl << "values_  ";
    for(int x = 0; x < (int)m_size; ++x){
        if(counts_[x] != 0)
            std::cout << ". ";
        else std::cout << "/ ";
