...
class DistancePixel {
 public:
  DistancePixel() : x(-1),y(-1),distance(std::numeric_limits<double>::max()), finalDistance(false) {}
  // accessor
  int getX() const { return x; }
  int getY() const { return y; }
  double getValue() const { return distance; }
  const bool finalDistancep() const {return finalDistance; }
  // modifier
  void setX(int _x) { x = _x; }
  void setY(int _y) { y = _y; }
  void setValue(double v) { distance = v; }
  void assertFinalDistance() { finalDistance = true; }
 private:
  // REPRESENTATION
  ...
  double distance;
  //whether that distance is the final one or not (instead og having a 2d array keeping track of such things)
  bool finalDistance;
};
