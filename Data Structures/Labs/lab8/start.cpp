...
bool  stepCanBeTaken(Point pnt, vector<Point> prevPnts){
    if(prevPnts.size() > 0){
        for(int x = 0; x < prevPnts.size(); x++){
            if(prevPnts[x].x == pnt.x && prevPnts[x].y == pnt.y)
                return false;
        }
        return true;
    }
...
void printPrevPnts(vector<Point> prevPnts){
    cout << "Path is:";
    for(int x = 0; x < prevPnts.size(); x++){
        cout << " (" << prevPnts[x].x << ", " << prevPnts[x].y << ")";
    }
    cout <<endl;
}

void stepToOrigin(Point pnt, vector<vector<bool> > blocked_grid, int &numPaths, vector<Point> prevPnts){
    Point origPnt = pnt;
    //cout << "got to this point, where pnt is " << pnt.x << ", " << pnt.y << endl;
    if(pnt.x > 0){
        if(blocked_grid[pnt.x - 1][pnt.y] == false){
            prevPnts.push_back(pnt);
            pnt.x = pnt.x - 1;
            if(stepCanBeTaken(pnt, prevPnts))
                stepToOrigin(pnt, blocked_grid, numPaths, prevPnts);
        }
    ...
    pnt = origPnt;
    if(pnt.y > 0){
        if(blocked_grid[pnt.x][pnt.y - 1] == false){
            prevPnts.push_back(pnt);
            pnt.y = pnt.y - 1;
            if(stepCanBeTaken(pnt, prevPnts))
                stepToOrigin(pnt, blocked_grid, numPaths, prevPnts);
...
int main(int argc, char* argv[]) {
  ...
  int numPaths = 0;
  Point startPnt(start_x, start_y);
  vector<Point> prevPnts;
  cout << "start point is: (" << start_x << ", " << start_y << ")" <<endl;
  stepToOrigin(startPnt, blocked_grid, numPaths, prevPnts);
  cout << "tot paths discovered: " << numPaths << endl;
