...
template <class T> unsigned int remove_matching_elements(Vec<T>& v, T elt){
    unsigned int matched_elts = 0;
    Vec<T> new_vec(0);
    for(unsigned int x = 0; x < v.size(); ++x){
        if(v[x] == elt)
            matched_elts++;
        else{
            new_vec.push_back(v[x]);
        }
    }
    Vec<T> temp_vec(new_vec.size());
    for(unsigned int x = 0; x < new_vec.size(); x++){
        temp_vec[x] = new_vec[x];
    }
    new_vec = temp_vec;
    v = new_vec;
    return matched_elts;
...
template <class T> void Vec<T>::print(){
    std::cout << "m_alloc: " << m_alloc << std::endl << "m_size: " << m_size << std::endl << "m_data: ";
