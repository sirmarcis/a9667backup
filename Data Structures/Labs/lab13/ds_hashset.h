...
public:
  ...
  if(it == this->end()){
    //m_table[it->index].push_back(key);
    ...
    //std::cout << "failed to put " << key << " in the hash, as " << *it << " is already there" <<std::endl;
...
private:
  ...
  //m_table.resize(new_size);
    std::vector< std::list<KeyType> > m_table2(new_size);
    for (unsigned int i=0; i<m_table.size(); ++i)
    {
      for (hash_list_itr p = m_table[i].begin(); p != m_table[i].end(); ++p)
      {
